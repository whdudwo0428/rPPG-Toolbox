...
15·30·20·40·10 s **가변 멀티윈도우**, **테일 패딩 정책(none/zero/edge/reflect)**, **윈도우·스트라이드 스윕 자동화(run\_sweep\_windows.py)**, 기존의 **미분가능 corr-loss**, **스케일/엔벨로프/분산 보조항**, **SNR-가중 학습**, **AMP-호환 gradient clipping**, 테스트 지표/플롯 **통합 저장**(+ 전실험 **exp\_results 집계**)
...
* **평가/저장**: **학습·평가·플롯을 한 번에** 수행(`run_train_lstm.py`). 스케일·부호 정렬 후 MSE/MAE, corr, corr\@soft-best-lag, Welch-BPM(0.08–0.60 Hz, **prominence + argmax 폴백**).
  → 결과는 **중첩 디렉토리**와 `runs/exp_results/` **CSV/JSON 집계**까지 자동 반영.
* **윈도우·스트라이드(V3)**: 윈도우 `{10,15,20,30,40}s`, 스트라이드 비율 `{0.10,0.20,0.25,0.50}`. **단일/2윈도우/3윈도우(20 고정)** 조합 전수 스윕 지원.
...
1. **가변 멀티윈도우/스트라이드 확대**
   ...
   * 윈도우: `{10, 15, 20, 30, 40}s`
   * 스트라이드(비율): `{0.10, 0.20, 0.25, 0.50}` (**0.80 제거**)
   * `STRIDE_FRAC_LIST`로 윈도우별 상이한 비율 지원. 절대초(`FIXED_STRIDE(_LIST)`)도 호환.
2. **테일 패딩 정책 유지/강화**: `WINDOW_PAD_MODE=none|zero|edge|reflect` + `WINDOW_INCLUDE_TAIL=1` → 말단 불완전 창까지 일관 커버.
3. **스윕 자동화 고도화**: `run_sweep_windows.py`가

   * **단일**: 5×4 전수
   * **2윈도우**: 짧은 `{10,15,20}` × 긴 `{30,40}` × 각자 스트라이드
   * **3윈도우(20 고정)**: (짧은 `{10,15}`) + 20 + (긴 `{30,40}`) × 각자 스트라이드
     를 실행하며, 각 런은 **중첩 디렉토리**로 저장되고 **exp\_results**에 자동 집계.
4. **학습·평가·플롯 통합 저장**: `run_train_lstm.py` 단일 실행으로
   `{best_model.pt, metrics.json, plots/*.png}` + **val/test 시간/지표**를 저장.
   → `runs/exp_results/{exp_results.csv, summary.csv, <run>.json}` **자동 누적**.
5. **평가/로깅 일원화**: Welch-BPM 옵션(폴백/보간/업샘플) 통일, SNR 가중/로그 스칼라 일관 기록.
...
→ Windows (`{10,15,20,30,40}s`), stride=`{0.10,0.20,0.25,0.50}` 또는 절대초/리스트
...
* α·|â−1| + γ·L1(envP,envG) + δ·|log(std(P)/std(G))|
  → Eval & Save: (scale·sign aligned) MSE/MAE, corr, corr\@bestlag, Welch-BPM(+fallback)
  → **중첩 디렉토리 저장 + exp\_results 집계**
...
* **통합 저장**: `run_train_lstm.py` 단일 실행 시

  * `{best_model.pt, metrics.json, plots/*.png}`를 **중첩 디렉토리**에 저장
  * 동시에 `runs/exp_results/`의

    * `exp_results.csv`(**long/세로**, 한 실행=한 행),
    * `summary.csv`(**wide/가로**, metric=행 / run=열),
    * `<run_name>.json`(per-run 상세)
      이 **자동 갱신**.
...
# --- 윈도우/스트라이드(V3) ---
export RR_WIN_LIST="15,30"                 # 예: 15s + 30s (임의 조합 가능: 10,15,20,30,40 중)
export STRIDE_FRAC="0.25"                  # 공통 비율, 또는
# export STRIDE_FRAC_LIST="0.25,0.50"      # 윈도우별 비율(0.10/0.20/0.25/0.50 중)
...
```
runs/<EXP_NAME>/model_rronly_time/<timestamp>/
  best_model.pt
  metrics.json           # {"val":{...},"test":{...}}
  metrics_row.csv        # 해당 런 1행(세로 포맷) 백업
  plots/test_00.png ...

runs/exp_results/
  exp_results.csv        # long(세로) 누적
  summary.csv            # wide(가로 pivot) 누적
  <run_name>.json        # per-run 상세(지표/시간/설정)
```

---

## 9) 윈도우/스트라이드 **스윕 자동화** (V3)

`cohface_exp_reg/run_sweep_windows.py`는 다음을 수행합니다.

* **전수 그리드**

  1. **단일**: 윈도우 `{10,15,20,30,40}` × 스트라이드 `{0.10,0.20,0.25,0.50}`
  2. **2윈도우**: 짧은 `{10,15,20}` × 긴 `{30,40}` × **각자 스트라이드**
  3. **3윈도우(20 고정)**: (짧은 `{10,15}`) + `20` + (긴 `{30,40}`) × **각자 스트라이드**
* **각 실험**: 환경변수(`RR_WIN_LIST`, `STRIDE_FRAC_LIST` 등)와 `BUCKET_BS` 세팅 → `run_train_lstm` 호출
* **저장/집계**: 각 런은 **중첩 디렉토리**에 저장되며, 동시에 `runs/exp_results/`의 `exp_results.csv`(long)·`summary.csv`(wide)·`<run>.json`이 자동 갱신
...
export RUN_GROUP=lstm_rronly   # EXP_NAME 접두
...
cohface_exp_reg/
  ...
  utils.py              # 필터/정규화/정렬/PSD-BPM + soft-corr(Torch/NumPy) + 집계 유틸
  ...
  run_extract_all.py
  run_train_lstm.py     # 학습+평가+플롯 통합 저장 (+ exp_results 집계)
  run_eval_best.py      # (옵션) 재평가/재플롯 + exp_results append
  run_sweep_windows.py  # 전수 스윕(단일/2윈도우/3윈도우)
